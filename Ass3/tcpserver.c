...

{
	...
	*/
	...
	serv_addr.sin_addr.s_addr	= INADDR_ANY;     // it sets this IP address as the address for server

	serv_addr.sin_port		= htons(20000);      // port number ( htons - network byte order )
	...
	*/
	...
	while (1) {
		...
		*/ 
		...
		// printf("checking the return value of recv fxn %d\n",check);
